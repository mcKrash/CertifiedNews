...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const size = 500; // Increased globe size
    ...
    <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden" style={{ background: 'linear-gradient(135deg, #f0fdfb 0%, #ccf2ef 100%)' }}>
      ...
      <div className="w-full lg:w-1/2 relative flex flex-col items-center justify-center p-8">
        ...
        <div className="mt-8 text-center animate-fade-in" style={{ animationDelay: '0.35s' }}>
          <h1 className="text-[24px] font-bold mb-2" style={{ color: '#1a2c3a' }}>World Certified News Alliance</h1>
          <p className="text-[15px]" style={{ color: '#5a6a7a' }}>Where We Make Sure It&apos;s Certified.</p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-[420px] bg-white/80 backdrop-blur-md p-10 rounded-[24px] shadow-xl animate-fade-in border border-white/20">
          <div className="flex flex-col items-center mb-10">
            <div className="mb-4">
              <Image src="/logo.png" alt="WCNA Logo" width={80} height={80} className="rounded-xl shadow-sm" />
            </div>
            <h2 className="text-[28px] font-extrabold mb-1" style={{ color: '#1a2c3a', letterSpacing: '-0.5px' }}>Welcome Back</h2>
            <p className="text-[15px]" style={{ color: '#8a9aaa' }}>Sign in to your account</p>
          </div>

          {message && <div className="p-4 rounded-xl text-sm mb-6" style={{ backgroundColor: '#E8F8F5', color: '#00B4A0' }}>{message}</div>}
          {error && <div className="p-4 rounded-xl text-sm mb-6" style={{ backgroundColor: '#FFE5E5', color: '#E74C3C' }}>{error}</div>}

          <form onSubmit={handleLogin} className="space-y-5 mb-8">
            <input type="text" placeholder="Username or email" value={emailOrUsername} onChange={(e) => setEmailOrUsername(e.target.value)} className="w-full px-5 py-4 border-[1.5px] rounded-[14px] text-sm transition-all focus:outline-none focus:border-[#00b8a0] focus:ring-[4px] focus:ring-[#00b8a0]/10" style={{ borderColor: '#dde3ea' }} required />
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-5 py-4 border-[1.5px] rounded-[14px] text-sm transition-all focus:outline-none focus:border-[#00b8a0] focus:ring-[4px] focus:ring-[#00b8a0]/10" style={{ borderColor: '#dde3ea' }} required />
            ...
            <button type="submit" disabled={loading} className="w-full py-4 rounded-[14px] font-bold text-white transition-all hover:translate-y-[-2px] active:translate-y-[0] disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #00c9af, #00b8a0)', boxShadow: '0 8px 24px rgba(0,184,160,0.3)' }}>{loading ? 'Signing in...' : 'Sign In'}</button>
          </form>

          <div className="relative mb-8">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t" style={{ borderColor: '#e8edf2' }}></div></div>
            <div className="relative flex justify-center text-[13px]"><span className="px-3 bg-white/0 text-[#aab4be]">Or continue with</span></div>
          </div>

          <div className="space-y-4">
            <button type="button" onClick={handleGoogleLogin} className="w-full py-4 border-[1.5px] rounded-[14px] font-bold flex items-center justify-center gap-3 hover:bg-white transition-all hover:shadow-md" style={{ borderColor: '#e0e6ed', color: '#1a2c3a' }}>
            ...
            <Link href="/auth/register" className="block w-full py-4 border-[1.5px] rounded-[14px] font-bold text-center hover:bg-white transition-all hover:shadow-md" style={{ borderColor: '#e0e6ed', color: '#1a2c3a' }}>Create Account</Link>
          </div>

          <p className="text-center text-[15px] mt-10" style={{ color: '#8a9aaa' }}>Don&apos;t have an account? <Link href="/auth/register" style={{ color: '#00b8a0', fontWeight: 'bold' }}>Create one</Link></p>
        </div>
      </div>
    </div>
  );
